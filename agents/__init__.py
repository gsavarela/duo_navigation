...
from agents.centralized import CentralizedActorCritic, FullyCentralizedActorCritic

def get_agent(env, flags):
    agent_cls = eval(flags.agent_type)
    if flags.agent_type in ('CentralizedActorCritic', 'FullyCentralizedActorCritic'):
...
__all__ = ['CentralizedActorCritic', 'FullyCentralizedActorCritic', 'SARSATabular', 'SARSASemiGradient']
