...
def snapshot_log(episode, env, agent, tr, log_dict, debug=True):
    ...
    if "task" not in log_dict:
        log_dict["task"] = agent.task
    if "label" not in log_dict:
        log_dict["label"] = agent.label

    if "average_reward" not in log_dict:
        avg_reward = reward
    else:
        prev = log_dict["average_reward"][-1]
        avg_reward = (prev * (total_step_count - 1) + reward) / total_step_count
...
def snapshot_state_actions_log(log, log_dir=None):
    ...
    df = pd.DataFrame.from_records(sac, columns=["state", "action", "count"]).pivot(
        index="state", columns="action", values="count"
    )
...
def snapshot_paths(log_dir, log=None, max_k_paths=10):
    ...
    def fn(x):
        return [str(tup) for tup in map(tuple, state2pos(x))]

    def gn(x):
        return np.nan if np.isnan(x) else ",".join(fn(x))
