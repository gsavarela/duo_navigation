...
def generate_transitions_from_moves(width=2, height=2):
    ...
    def getpos(x):
        return state2pos(x, n_agents=2, width=width, height=height)

    # Get state from positions
    def getstate(x):
        return pos2state(x, n_agents=2, width=width, height=height)
    ...
    for state in range(16):
        ...
        for ii, ag_moves in enumerate(action_set(2)):
